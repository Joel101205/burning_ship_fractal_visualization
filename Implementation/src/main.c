...
void helpMessage() {
    ...
    printf(
        "Usage: ./BurningShip [-V (int)] [-B (int)] [-o (String)] [-s (float),(float)] [-d (size_t),(size_t)] [-n (int)] [-r (float)] \n");
    ...
    printf(
        "                                  0: Assembly SIMD Implementation, 1: Assembly SISD Implementation, 2: C SISD Implementation: 2, 3: C compiler optimized\n");
    printf(
        " -B(optionalInt)                : when set, measures benchmark time. Optional number parameter sets number of repetitions \n");
    printf(" -o (String)                    : Sets the name of the output file. Default Name: burning_ship.bmp\n");
    printf(
        " -s (float),(float)             : Sets the starting point (top left) for the calculation, 1. real part, 2. imaginary part; Default: -1.8,0.01\n");
    printf(
        " -d (size_t),(size_t)           : Sets the dimensions for the output image: 1. width, 2. height; Default:1920,1080 \n");
...
int main(int argc, char *argv[]) {
    ...
    if (benchmark > 0) burning_ship(complexStart, imgWidth, imgHeight, res, n, img);
    // warmup execution for benchmarking
